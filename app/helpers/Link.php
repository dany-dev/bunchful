    ...
    public static function get_biolink_link($link, $user = null, $biolink_theme = null) {
        ...
        switch($link->type) {
            ...
            case 'service':
                ...
                $link->design->link_style = 'background: ' . $link->settings->background_color . ';color: ' . $link->settings->text_color . ';border-width: ' . ($link->settings->border_width ?? '1') . 'px; border-color: ' . ($link->settings->border_color ?? 'transparent') . ';border-style: ' . ($link->settings->border_style ?? 'solid') . ';';
