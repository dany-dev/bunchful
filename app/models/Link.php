    ...
    public function delete($link_id) {
        ...
        if($link->type == 'file') {
            ...
            if(\Altum\Plugin::is_active('offload') && settings()->offload->uploads_url) {
                ...
                $s3->deleteObject([
                    'Bucket' => settings()->offload->storage_name,
                    'Key' => 'uploads/files/' . $link->settings->file,
                ]);
            }
            ...
            /* Local deleting */
                ...
                /* Delete favicon */
                if(!empty($link->settings->file) && file_exists(UPLOADS_PATH . 'files/' . $link->settings->file)) {
                    unlink(UPLOADS_PATH . 'files/' . $link->settings->file);
                }
            }
        }
