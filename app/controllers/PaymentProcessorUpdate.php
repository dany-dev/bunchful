...
class PaymentProcessorUpdate extends Controller {
    ...
    public function index() {
        ...
        if(!empty($_POST)) {
            ...
            $_POST['processor'] = isset($_POST['processor']) && in_array($_POST['processor'], ['paypal', 'stripe', 'crypto_com', 'razorpay', 'paystack']) ? Database::clean_string($_POST['processor']) : 'https://';
            ...
            switch($_POST['processor']) {
                ...
                case 'paystack':
                    $settings['public_key'] = $_POST['public_key'] = filter_var($_POST['public_key'], FILTER_SANITIZE_STRING);
                    $settings['secret_key'] = $_POST['secret_key'] = filter_var($_POST['secret_key'], FILTER_SANITIZE_STRING);
                    break;

                case 'razorpay':
                    $settings['key_id'] = $_POST['key_id'] = filter_var($_POST['key_id'], FILTER_SANITIZE_STRING);
                    $settings['key_secret'] = $_POST['key_secret'] = filter_var($_POST['key_secret'], FILTER_SANITIZE_STRING);
                    $settings['webhook_secret'] = $_POST['webhook_secret'] = filter_var($_POST['webhook_secret'], FILTER_SANITIZE_STRING);
                    break;
            }
