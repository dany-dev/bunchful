...
class Link extends Controller {
    ...
    public function index() {
        ...
        if(!$has_access && !$this->is_preview) {
            ...
            if($this->link->type == 'biolink') {
                ...
                $vcard->addPhoneNumber($this->link->settings->vcard_phone);
    ...
    public function mail() {
        ...
        $_POST['name'] = mb_substr(trim(Database::clean_string($_POST['name'])), 0, 32);

        if(settings()->captcha->type != 'basic' && !(new Captcha())->is_valid()) {
    ...
    public function phone_collector() {
        ...
        $_POST['biolink_block_id'] = (int) $_POST['biolink_block_id'];
        $_POST['phone'] = mb_substr(trim(Database::clean_string($_POST['phone'])), 0, 32);
        $_POST['name'] = mb_substr(trim(Database::clean_string($_POST['name'])), 0, 32);
    ...
    public function payment_generator() {
        ...
        switch($biolink_block->type) {
            case 'donation':
                ...
                $data['message'] = $_POST['message'] = mb_substr(trim(Database::clean_string($_POST['message'] ?? null)), 0, 256);
                break;

            case 'product':
                $price = $_POST['price'] = (float) $_POST['price'];
                $email = $_POST['email'] = mb_substr(trim(Database::clean_string($_POST['email'] ?? null)), 0, 320);
                break;
