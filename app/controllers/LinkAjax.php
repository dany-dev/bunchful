...
class LinkAjax extends Controller
{
    ...
    private function create()
    {
        ...
        if (!array_key_exists($_POST['type'], require APP_PATH . 'includes/links_types.php')) {
            die();
        }
    ...
    private function create_biolink_template()
    {
    ...
    private function create_file()
    {
        ...
        $_POST['url'] = !empty($_POST['url']) ? get_slug(Database::clean_string($_POST['url']), '-', false) : null;

        if (empty($_POST['domain_id']) && !settings()->links->main_domain_is_enabled && !\Altum\Middlewares\Authentication::is_admin()) {
            Response::json(l('create_file_modal.error_message.main_domain_is_disabled'), 'error');
        }
        ...
        /* File upload */
        require_once APP_PATH . 'controllers/BiolinkBlockAjax.php';
        $biolink_block_ajax = (new \Altum\Controllers\BiolinkBlockAjax());
        $biolink_block_ajax->add_params(['user' => $this->user]);
        $db_file = $biolink_block_ajax->handle_file_upload(null, 'file', 'file_remove', \Altum\Uploads::get_whitelisted_file_extensions('files'), 'files/', settings()->links->file_size_limit);
        ...
        if ($_POST['url']) {
            if (db()->where('url', $_POST['url'])->where('domain_id', $domain_id)->getValue('links', 'link_id')) {
                Response::json(l('create_file_modal.error_message.url_exists'), 'error');
            }
        }
    ...
    private function update_biolink()
    {
        ...
        $image_allowed_extensions = [
            ...
            'favicon' => ['png', 'gif', 'ico'],
            'background' => ['jpg', 'jpeg', 'png', 'svg', 'gif', 'mp4']
        ];
        $image = [
        ...
        Response::json(l('global.success_message.update2'), 'success', [
            ...
            'background_url' => $image_url['background'],
        ]);
    }
    ...
    private function update_file()
    {
        ...
        $biolink_block_ajax = (new \Altum\Controllers\BiolinkBlockAjax());
        $biolink_block_ajax->add_params(['user' => $this->user]);
        $db_file = $biolink_block_ajax->handle_file_upload($link->settings->file, 'file', 'file_remove', \Altum\Uploads::get_whitelisted_file_extensions('files'), 'files/', settings()->links->file_size_limit);
    ...
    private function update_vcard()
    {
        ...
        $biolink_block_ajax = (new \Altum\Controllers\BiolinkBlockAjax());
        $biolink_block_ajax->add_params(['user' => $this->user]);
        $db_vcard_avatar = $biolink_block_ajax->handle_file_upload($link->settings->vcard_avatar, 'vcard_avatar', 'vcard_avatar_remove', \Altum\Uploads::get_whitelisted_file_extensions('vcards_avatars'), 'avatars/', 0.75);
        ...
        if (isset($_POST['vcard_avatar_remove'])) {
            ...
            if (\Altum\Plugin::is_active('offload') && settings()->offload->uploads_url) {
                ...
                $s3->deleteObject([
                    'Bucket' => settings()->offload->storage_name,
                    'Key' => 'uploads/avatars/' . $link->settings->vcard_avatar,
                ]);
            }

            /* Local deleting */ else {
                /* Delete current file */
                if (!empty($link->settings->vcard_avatar) && file_exists(UPLOADS_PATH . 'avatars/' . $link->settings->vcard_avatar)) {
                    unlink(UPLOADS_PATH . 'avatars/' . $link->settings->vcard_avatar);
                }
            }
            $db_vcard_avatar = null;
        }
        ...
        $settings['vcard_first_name'] = $_POST['vcard_first_name'] = mb_substr(trim(Database::clean_string($_POST['vcard_first_name'])), 0, $links_types['vcard']['fields']['first_name']['max_length']);
        $settings['vcard_last_name'] = $_POST['vcard_last_name'] = mb_substr(trim(Database::clean_string($_POST['vcard_last_name'])), 0, $links_types['vcard']['fields']['last_name']['max_length']);
        $settings['vcard_phone'] = $_POST['vcard_phone'] = mb_substr(trim(Database::clean_string($_POST['vcard_phone'])), 0, $links_types['vcard']['fields']['phone']['max_length']);
        $settings['vcard_email'] = $_POST['vcard_email'] = mb_substr(trim(Database::clean_string($_POST['vcard_email'])), 0, $links_types['vcard']['fields']['email']['max_length']);
        $settings['vcard_url'] = $_POST['vcard_url'] = mb_substr(trim(Database::clean_string($_POST['vcard_url'])), 0, $links_types['vcard']['fields']['url']['max_length']);
        $settings['vcard_company'] = $_POST['vcard_company'] = mb_substr(trim(Database::clean_string($_POST['vcard_company'])), 0, $links_types['vcard']['fields']['company']['max_length']);
        $settings['vcard_job_title'] = $_POST['vcard_job_title'] = mb_substr(trim(Database::clean_string($_POST['vcard_job_title'])), 0, $links_types['vcard']['fields']['job_title']['max_length']);
        $settings['vcard_birthday'] = $_POST['vcard_birthday'] = mb_substr(trim(Database::clean_string($_POST['vcard_birthday'])), 0, $links_types['vcard']['fields']['birthday']['max_length']);
        $settings['vcard_street'] = $_POST['vcard_street'] = mb_substr(trim(Database::clean_string($_POST['vcard_street'])), 0, $links_types['vcard']['fields']['street']['max_length']);
        $settings['vcard_city'] = $_POST['vcard_city'] = mb_substr(trim(Database::clean_string($_POST['vcard_city'])), 0, $links_types['vcard']['fields']['city']['max_length']);
        $settings['vcard_zip'] = $_POST['vcard_zip'] = mb_substr(trim(Database::clean_string($_POST['vcard_zip'])), 0, $links_types['vcard']['fields']['zip']['max_length']);
        $settings['vcard_region'] = $_POST['vcard_region'] = mb_substr(trim(Database::clean_string($_POST['vcard_region'])), 0, $links_types['vcard']['fields']['region']['max_length']);
        $settings['vcard_country'] = $_POST['vcard_country'] = mb_substr(trim(Database::clean_string($_POST['vcard_country'])), 0, $links_types['vcard']['fields']['country']['max_length']);
        $settings['vcard_note'] = $_POST['vcard_note'] = mb_substr(trim(Database::clean_string($_POST['vcard_note'])), 0, $links_types['vcard']['fields']['note']['max_length']);
        ...
        foreach ($_POST['vcard_social_label'] as $key => $value) {
            ...
            $vcard_socials[] = [
                'label' => mb_substr(trim(Database::clean_string($value)), 0, $links_types['vcard']['fields']['social_value']['max_length']),
                'value' => mb_substr(trim(filter_var($_POST['vcard_social_value'][$key], FILTER_SANITIZE_STRING)), 0, $links_types['vcard']['fields']['social_value']['max_length'])
            ];
        }
    ...
    public function duplicate()
    {
        ...
        if (!Alerts::has_field_errors() && !Alerts::has_errors()) {
            ...
            if ($link->type == 'vcard') {
                $link->settings->vcard_avatar = null;
            }

            if ($link->type == 'file') {
                $link->settings->file = null;
            }
