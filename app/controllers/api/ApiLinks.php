...
class ApiLinks extends Controller {
    ...
    private function post() {
        ...
        /* Type of link */
        $_POST['type'] = in_array($_POST['type'], ['link']) ? Database::clean_string($_POST['type']) : null;
        ...
        if($_POST['url']) {
            if(db()->where('url', $_POST['url'])->where('domain_id', $domain_id)->getValue('links', 'link_id')) {
                $this->response_error(l('create_link_modal.error_message.url_exists'), 401);
            }
        ...
        $_POST['targeting_type'] = isset($_POST['targeting_type']) && in_array($_POST['targeting_type'], array_merge(['false'], $targeting_types)) ? Database::clean_string($_POST['targeting_type']) : 'false';
    ...
    private function patch() {
        ...
        if(isset($_POST['domain_id']) && $_POST['domain_id'] == 0 && !settings()->links->main_domain_is_enabled && !\Altum\Middlewares\Authentication::is_admin()) {
            $this->response_error(l('create_link_modal.error_message.main_domain_is_disabled'), 401);
        }
        ...
        /* Location & url */
        $_POST['location_url'] = mb_substr(trim(Database::clean_string($_POST['location_url'] ?? $link->location_url)), 0, 2048);
        $_POST['url'] = !empty($_POST['url']) ? get_slug(Database::clean_string($_POST['url'] ?? $link->url), '-', false) : false;
        $this->check_url($_POST['url']);
        $this->check_location_url($_POST['location_url']);
        ...
        $_POST['expiration_url'] = mb_substr(trim(Database::clean_string($_POST['expiration_url'] ?? $link->settings->expiration_url)), 0, 2048);
        ...
        /* Prepare the settings */
        $targeting_types = ['country_code', 'device_type', 'browser_language', 'rotation', 'os_name'];
        $_POST['targeting_type'] = isset($_POST['targeting_type']) && in_array($_POST['targeting_type'], array_merge(['false'], $targeting_types)) ? Database::clean_string($_POST['targeting_type']) : $link->settings->targeting_type;
