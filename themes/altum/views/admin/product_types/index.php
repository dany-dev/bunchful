...
<div class="d-flex flex-column flex-md-row justify-content-between mb-4">
    <h1 class="h3 m-0"><i class="fa fa-fw fa-xs fa-bars text-primary-900 mr-2"></i> <?= l('admin_product_types.header') ?></h1>

    <div class="d-flex position-relative">
        <div class="">
            <button class="btn btn-outline-primary" data-toggle="modal" data-target="#product_type_create_modal">
                <i class="fa fa-fw fa-plus-circle"></i> <?= l('admin_product_types.create') ?>
            </button>
        </div>
        <div class="ml-3">
            <div class="dropdown">
                ...
                <div class="dropdown-menu dropdown-menu-right d-print-none">
                    <a href="<?= url('admin/product_types?' . $data->filters->get_get() . '&export=csv') ?>" target="_blank" class="dropdown-item">
                        <i class="fa fa-fw fa-sm fa-file-csv mr-1"></i> <?= l('global.export_csv') ?>
                    </a>
                    <a href="<?= url('admin/product_types?' . $data->filters->get_get() . '&export=json') ?>" target="_blank" class="dropdown-item">
                ...
                <div class="dropdown-menu dropdown-menu-right filters-dropdown">
                    <div class="dropdown-header d-flex justify-content-between">
                        ...
                        <?php if (count($data->filters->get)) : ?>
                            <a href="<?= url('admin/product_types') ?>" class="text-muted"><?= l('global.filters.reset') ?></a>
                        <?php endif ?>
                    </div>
...
<form id="table" action="<?= SITE_URL . 'admin/product-types/bulk' ?>" method="post" role="form">
        ...
        <table class="table table-custom">
            <thead>
                <tr>
                    ...
                    <th><?= l('admin_product_types.table.name') ?></th>
                    <th><?= l('admin_product_types.table.datetime') ?></th>
                    <th></th>
                </tr>
            </thead>
            <tbody>
                <?php foreach ($data->product_types as $row) : ?>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="d-flex justify-content-end">
                                <?= include_view(THEME_PATH . 'views/admin/product_types/admin_product_type_dropdown_button.php', ['id' => $row->product_type_id, 'resource_name' => $row->name]) ?>
                            </div>
                        </td>
                    </tr>
                <?php endforeach ?>
...
<?php \Altum\Event::add_content(include_view(THEME_PATH . 'views/admin/product_types/add_product_type_modal.php'), 'modals'); ?>
