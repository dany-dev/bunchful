...
<div class="d-flex flex-column flex-md-row justify-content-between mb-4">
    <h1 class="h3 m-0"><i class="fa fa-fw fa-xs fa-bars text-primary-900 mr-2"></i> <?= l('admin_catalog.header') ?></h1>

    <div class="d-flex position-relative">
        <div class="">
            <button class="btn btn-outline-primary" data-toggle="modal" data-target="#catalog_create_modal">
                <i class="fa fa-fw fa-plus-circle"></i> <?= l('admin_catalog.create') ?>
            </button>
        </div>
        <div class="ml-3">
            <div class="dropdown">
                ...
                <div class="dropdown-menu dropdown-menu-right d-print-none">
                    <a href="<?= url('admin/catalog?' . $data->filters->get_get() . '&export=csv') ?>" target="_blank" class="dropdown-item">
                        <i class="fa fa-fw fa-sm fa-file-csv mr-1"></i> <?= l('global.export_csv') ?>
                    </a>
                    <a href="<?= url('admin/catalog?' . $data->filters->get_get() . '&export=json') ?>" target="_blank" class="dropdown-item">
                ...
                <div class="dropdown-menu dropdown-menu-right filters-dropdown">
                    <div class="dropdown-header d-flex justify-content-between">
                        ...
                        <?php if (count($data->filters->get)) : ?>
                            <a href="<?= url('admin/catalog') ?>" class="text-muted"><?= l('global.filters.reset') ?></a>
                        <?php endif ?>
                    </div>
...
<form id="table" action="<?= SITE_URL . 'admin/catalog/bulk' ?>" method="post" role="form">
        ...
        <table class="table table-custom">
            <thead>
                <tr>
                    ...
                    <th><?= l('admin_catalog.table.name') ?></th>
                    <th><?= l('admin_catalog.table.description') ?></th>
                    <th><?= l('admin_catalog.table.price') ?></th>
                    <th><?= l('admin_catalog.table.datetime') ?></th>
                    <th></th>
                </tr>
            </thead>
            <tbody>
                <?php foreach ($data->catalog as $row) : ?>
                    ...
                    <tr>
                        <td data-bulk-table class="d-none">
                            <div class="custom-control custom-checkbox">
                                <input id="selected_catalog_id_<?= $row->catalog_id ?>" type="checkbox" class="custom-control-input" name="selected[]" value="<?= $row->catalog_id ?>" />
                                <label class="custom-control-label" for="selected_catalog_id_<?= $row->catalog_id ?>"></label>
                            </div>
                        </td>
                        ...
                        <td>
                            <div class="d-flex justify-content-end">
                                <?= include_view(THEME_PATH . 'views/admin/catalog/admin_catalog_dropdown_button.php', ['id' => $row->catalog_id, 'resource_name' => $row->name]) ?>
                            </div>
                        </td>
                    </tr>
                <?php endforeach ?>
...
<?php \Altum\Event::add_content(include_view(THEME_PATH . 'views/admin/catalog/add_catalog_modal.php'), 'modals'); ?>
