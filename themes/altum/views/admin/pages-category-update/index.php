...
<nav aria-label="breadcrumb">
    <ol class="custom-breadcrumbs small">
        ...
        <li class="active" aria-current="page"><?= l('admin_pages_category_update.breadcrumb') ?></li>
    </ol>
</nav>

<div class="d-flex justify-content-between mb-4">
    <h1 class="h3 mb-0 text-truncate"><i class="fa fa-fw fa-xs fa-book text-primary-900 mr-2"></i> <?= l('admin_pages_category_update.header') ?></h1>

    <?= include_view(THEME_PATH . 'views/admin/pages-categories/admin_pages_category_dropdown_button.php', ['id' => $data->pages_category->pages_category_id, 'resource_name' => $data->pages_category->title]) ?>
</div>
        ...
        <form action="" method="post" role="form">
            ...
            <div class="form-group">
                ...
                <div class="input-group">
                    ...
                    <input id="url" type="text" name="url" class="form-control form-control-lg <?= \Altum\Alerts::has_field_errors('url') ? 'is-invalid' : null ?>" placeholder="<?= l('admin_resources.main.url_placeholder') ?>" value="<?= $data->pages_category->url ?>" required="required" />
            ...
            <div class="form-group">
                ...
                <input id="icon" type="text" name="icon" class="form-control form-control-lg" placeholder="<?= l('admin_pages_categories.input.icon_placeholder') ?>" value="<?= $data->pages_category->icon ?>" />
            ...
            <div class="form-group">
                ...
                <select id="language" name="language" class="form-control form-control-lg">
                    <option value="" <?= !$data->pages_category->language ? 'selected="selected"' : null ?>><?= l('admin_resources.main.language_all') ?></option>
                    <?php foreach(\Altum\Language::$languages as $language): ?>
                        <option value="<?= $language['name'] ?>" <?= $data->pages_category->language == $language['name'] ? 'selected="selected"' : null ?>><?= $language['name'] ?></option>
                    <?php endforeach ?>
                </select>
            </div>
