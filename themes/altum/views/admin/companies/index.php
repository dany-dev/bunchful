...
<div class="d-flex flex-column flex-md-row justify-content-between mb-4">
    <h1 class="h3 m-0"><i class="fa fa-fw fa-xs fas fa-building text-primary-900 mr-2"></i> <?= l('admin_companies.header') ?></h1>
        ...
        <div class="ml-3">
            <div class="dropdown">
                ...
                <div class="dropdown-menu dropdown-menu-right d-print-none">
                    <a href="<?= url('admin/companies?' . $data->filters->get_get() . '&export=csv') ?>" target="_blank" class="dropdown-item">
                        <i class="fa fa-fw fa-sm fa-file-csv mr-1"></i> <?= l('global.export_csv') ?>
                    </a>
                    <a href="<?= url('admin/companies?' . $data->filters->get_get() . '&export=json') ?>" target="_blank" class="dropdown-item">
                ...
                <div class="dropdown-menu dropdown-menu-right filters-dropdown">
                    <div class="dropdown-header d-flex justify-content-between">
                        ...
                        <?php if (count($data->filters->get)) : ?>
                            <a href="<?= url('admin/companies') ?>" class="text-muted"><?= l('global.filters.reset') ?></a>
                        <?php endif ?>
                    </div>
                    ...
                    <form action="" method="get" role="form">
                        ...
                        <div class="form-group px-4">
                            ...
                            <select name="search_by" id="filters_search_by" class="form-control form-control-sm">
                                <option value="name" <?= $data->filters->search_by == 'name' ? 'selected="selected"' : null ?>><?= l('admin_companies.table.name') ?></option>
                                <option value="product_id" <?= $data->filters->search_by == 'product_id' ? 'selected="selected"' : null ?>><?= l('admin_companies.table.product_id') ?></option>
                            </select>
                        </div>

                        <div class="form-group px-4">
                            <label for="filters_user_id" class="small"><?= l('global.filters.user') ?></label>
                            <select name="user_id" id="filters_user_id" class="form-control form-control-sm">
                                <option value=""><?= l('global.filters.all') ?></option>
                                <?php foreach ($data->product_users as $user_key => $user) : ?>
                                    <option value="<?= $user_key ?>" <?= isset($data->filters->filters['user_id']) && $data->filters->filters['user_id'] == $user_key ? 'selected="selected"' : null ?>><?= $user ?></option>
                                <?php endforeach ?>
                            </select>
                        </div>
                        ...
                        <div class="form-group px-4">
                            ...
                            <select name="order_by" id="filters_order_by" class="form-control form-control-sm">
                                ...
                                <option value="name" <?= $data->filters->order_by == 'name' ? 'selected="selected"' : null ?>><?= l('admin_companies.table.name') ?></option>
                            </select>
                        </div>
...
<form id="table" action="<?= SITE_URL . 'admin/products/bulk' ?>" method="post" role="form">
        ...
        <table class="table table-custom">
            <thead>
                <tr>
                    ...
                    <th><?= l('admin_companies.table.name') ?></th>
                    <th><?= l('admin_companies.table.group_owner') ?></th>
                    <th><?= l('admin_companies.table.users') ?></th>
                    <th><?= l('admin_companies.table.datetime') ?></th>
                    <th></th>
                </tr>
            </thead>
            <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="d-flex justify-content-end">
                                <?= include_view(THEME_PATH . 'views/admin/companies/admin_company_dropdown_button.php', ['id' => $row->id, 'resource_name' => $row->name]) ?>
                            </div>
                        </td>
                    </tr>
                <?php endforeach ?>
