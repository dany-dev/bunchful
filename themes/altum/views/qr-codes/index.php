                    ...
                    <div class="col-2 col-lg-2 d-flex justify-content-center justify-content-lg-end align-items-center">
                        <div class="dropdown">
                            ...
                            <div class="dropdown-menu">
                                <a href="<?= UPLOADS_FULL_URL . 'qr_code/' . $row->qr_code ?>" class="dropdown-item" download="<?= get_slug($row->name) . '.svg' ?>"><?= l('qr_codes.download_svg') ?></a>
                                <button type="button" class="dropdown-item" onclick="convert_svg_to_others('<?= UPLOADS_FULL_URL . 'qr_code/' . $row->qr_code ?>', 'png', '<?= get_slug($row->name) . '.png' ?>');"><?= l('qr_codes.download_png') ?></button>
                                <button type="button" class="dropdown-item" onclick="convert_svg_to_others('<?= UPLOADS_FULL_URL . 'qr_code/' . $row->qr_code ?>', 'jpg', '<?= get_slug($row->name) . '.jpg' ?>');"><?= l('qr_codes.download_jpg') ?></button>
                                <button type="button" class="dropdown-item" onclick="convert_svg_to_others('<?= UPLOADS_FULL_URL . 'qr_code/' . $row->qr_code ?>', 'webp', '<?= get_slug($row->name) . '.webp' ?>');"><?= l('qr_codes.download_webp') ?></button>
                            </div>
                        </div>
