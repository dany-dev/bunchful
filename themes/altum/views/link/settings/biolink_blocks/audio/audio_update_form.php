...
<form name="update_biolink_" method="post" role="form" enctype="multipart/form-data">
    ...
    <div class="form-group">
        <label for="<?= 'audio_file_' . $row->biolink_block_id ?>"><i class="fa fa-fw fa-volume-up fa-sm text-muted mr-1"></i> <?= l('create_biolink_audio_modal.file') ?></label>
        <input id="<?= 'audio_file_' . $row->biolink_block_id ?>" type="file" name="file" accept="<?= implode(', ', array_map(function($value) { return '.' . $value; }, $data->biolink_blocks['audio']['whitelisted_file_extensions'])) ?>" class="form-control-file altum-file-input" />
        <small class="form-text text-muted"><?= l('create_biolink_audio_modal.file_help') ?></small>
    </div>

    <div class="form-group">
        <label for="<?= 'audio_name_' . $row->biolink_block_id ?>"><i class="fa fa-fw fa-signature fa-sm text-muted mr-1"></i> <?= l('create_biolink_link_modal.input.name') ?></label>
        <input id="<?= 'audio_name_' . $row->biolink_block_id ?>" type="text" name="name" class="form-control" value="<?= $row->settings->name ?>" maxlength="128" required="required" />
    </div>
