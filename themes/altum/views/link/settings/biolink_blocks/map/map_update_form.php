...
<form name="update_biolink_" method="post" role="form" enctype="multipart/form-data">
    ...
    <div class="form-group">
        <label for="<?= 'map_address_' . $row->biolink_block_id ?>"><i class="fa fa-fw fa-map-marker-alt fa-sm text-muted mr-1"></i> <?= l('create_biolink_map_modal.address') ?></label>
        <input id="<?= 'map_address_' . $row->biolink_block_id ?>" type="text" class="form-control" name="address" value="<?= $row->settings->address ?>" maxlength="64" required="required" />
    </div>

    <div class="form-group">
        <label for="<?= 'map_location_url_' . $row->biolink_block_id ?>"><i class="fa fa-fw fa-link fa-sm text-muted mr-1"></i> <?= l('create_biolink_link_modal.input.location_url') ?></label>
        <input id="<?= 'map_location_url_' . $row->biolink_block_id ?>" type="text" class="form-control" name="location_url" value="<?= $row->location_url ?>" maxlength="2048" />
    </div>

    <div class="form-group">
        <label for="<?= 'map_zoom_' . $row->biolink_block_id ?>"><i class="fa fa-fw fa-search fa-sm text-muted mr-1"></i> <?= l('create_biolink_map_modal.zoom') ?></label>
        <input id="<?= 'map_zoom_' . $row->biolink_block_id ?>" type="range"  min="1" max="20" class="form-control" name="zoom" value="<?= $row->settings->zoom ?>" required="required" />
    </div>

    <div class="form-group">
        <label for="<?= 'map_type_' . $row->biolink_block_id ?>"><i class="fa fa-fw fa-map fa-sm text-muted mr-1"></i> <?= l('create_biolink_map_modal.type') ?></label>
        <select id="<?= 'map_type_' . $row->biolink_block_id ?>" name="type" class="form-control">
            <option value="roadmap" <?= $row->settings->type == 'roadmap' ? 'selected="selected"' : null ?>><?= l('create_biolink_map_modal.type.roadmap') ?></option>
            <option value="satellite" <?= $row->settings->type == 'satellite' ? 'selected="selected"' : null ?>><?= l('create_biolink_map_modal.type.satellite') ?></option>
            <option value="terrain" <?= $row->settings->type == 'terrain' ? 'selected="selected"' : null ?>><?= l('create_biolink_map_modal.type.terrain') ?></option>
            <option value="hybrid" <?= $row->settings->type == 'hybrid' ? 'selected="selected"' : null ?>><?= l('create_biolink_map_modal.type.hybrid') ?></option>
        </select>
    </div>
