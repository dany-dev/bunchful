...
<form name="update_biolink_" method="post" role="form">
    ...
    <div class="form-group">
        <label for="<?= 'paypal_type_' . $row->biolink_block_id ?>"><i class="fab fa-fw fa-paypal fa-sm text-muted mr-1"></i> <?= l('create_biolink_paypal_modal.type') ?></label>
        <select id="<?= 'paypal_type_' . $row->biolink_block_id ?>" name="type" class="form-control">
            <?php foreach(['buy_now', 'add_to_cart', 'donation'] as $key): ?>
                <option value="<?= $key ?>" <?= $row->settings->type == $key ? 'selected="selected"' : null ?>><?= l('create_biolink_paypal_modal.type_' . $key) ?></option>
            <?php endforeach ?>
        </select>
    </div>

    <div class="form-group">
        <label for="<?= 'paypal_email_' . $row->biolink_block_id ?>"><i class="fa fa-fw fa-envelope fa-sm text-muted mr-1"></i> <?= l('create_biolink_paypal_modal.email') ?></label>
        <input id="<?= 'paypal_email_' . $row->biolink_block_id ?>" type="email" class="form-control" name="email" required="required" value="<?= $row->settings->email ?>" maxlength="320" />
    </div>

    <div class="form-group">
        <label for="<?= 'paypal_title_' . $row->biolink_block_id ?>"><i class="fa fa-fw fa-heading fa-sm text-muted mr-1"></i> <?= l('create_biolink_paypal_modal.title') ?></label>
        <input id="<?= 'paypal_title_' . $row->biolink_block_id ?>" type="text" name="title" maxlength="256" class="form-control" value="<?= $row->settings->title ?>" required="required" />
    </div>

    <div class="form-group">
        <label for="<?= 'paypal_currency_' . $row->biolink_block_id ?>"><i class="fa fa-fw fa-euro-sign fa-sm text-muted mr-1"></i> <?= l('create_biolink_paypal_modal.currency') ?></label>
        <input id="<?= 'paypal_currency_' . $row->biolink_block_id ?>" type="text" name="currency" maxlength="8" class="form-control" value="<?= $row->settings->currency ?>" placeholder="<?= l('create_biolink_paypal_modal.currency_placeholder') ?>" required="required" />
    </div>

    <div class="form-group">
        <label for="<?= 'paypal_price_' . $row->biolink_block_id ?>"><i class="fa fa-fw fa-dollar-sign fa-sm text-muted mr-1"></i> <?= l('create_biolink_paypal_modal.price') ?></label>
        <input id="<?= 'paypal_price_' . $row->biolink_block_id ?>" type="number" name="price" min="1" step="0.01" class="form-control" value="<?= $row->settings->price ?>" required="required" />
    </div>

    <div class="form-group">
        <label for="<?= 'paypal_thank_you_url_' . $row->biolink_block_id ?>"><i class="fa fa-fw fa-link fa-sm text-muted mr-1"></i> <?= l('create_biolink_paypal_modal.thank_you_url') ?></label>
        <input id="<?= 'paypal_thank_you_url_' . $row->biolink_block_id ?>" type="url" name="thank_you_url" maxlength="2048" class="form-control" value="<?= $row->settings->thank_you_url ?>" />
    </div>

    <div class="form-group">
        <label for="<?= 'paypal_cancel_url_' . $row->biolink_block_id ?>"><i class="fa fa-fw fa-link fa-sm text-muted mr-1"></i> <?= l('create_biolink_paypal_modal.cancel_url') ?></label>
        <input id="<?= 'paypal_cancel_url_' . $row->biolink_block_id ?>" type="url" name="cancel_url" maxlength="2048" class="form-control" value="<?= $row->settings->cancel_url ?>" />
    </div>

    <div class="form-group">
        <label for="<?= 'paypal_name_' . $row->biolink_block_id ?>"><i class="fa fa-fw fa-signature fa-sm text-muted mr-1"></i> <?= l('create_biolink_link_modal.input.name') ?></label>
        <input id="<?= 'paypal_name_' . $row->biolink_block_id ?>" type="text" name="name" class="form-control" value="<?= $row->settings->name ?>" maxlength="128" required="required" />
    </div>

    <div class="custom-control custom-switch mb-3">
        <input
                id="<?= 'paypal_open_in_new_tab_' . $row->biolink_block_id ?>"
                name="open_in_new_tab" type="checkbox"
                class="custom-control-input"
            <?= $row->settings->open_in_new_tab ? 'checked="checked"' : null ?>
        >
        <label class="custom-control-label" for="<?= 'paypal_open_in_new_tab_' . $row->biolink_block_id ?>"><?= l('create_biolink_link_modal.input.open_in_new_tab') ?></label>
    </div>
    ...
    <div class="form-group">
        ...
        <input id="<?= 'link_image_' . $row->biolink_block_id ?>" type="file" name="image" accept=".gif, .png, .jpg, .jpeg, .svg" class="form-control-file altum-file-input" />
    </div>
    ...
    <div <?= $this->user->plan_settings->custom_colored_links ? null : 'data-toggle="tooltip" title="' . l('global.info_message.plan_feature_no_access') . '"' ?>>
        <div class="<?= $this->user->plan_settings->custom_colored_links ? null : 'container-disabled' ?>">
            ...
            <div class="form-group">
                ...
                <input id="<?= 'link_border_width_' . $row->biolink_block_id ?>" type="range" min="0" max="5" class="form-control" name="border_width" value="<?= $row->settings->border_width ?>" required="required" />
            </div>
            ...
            <div class="form-group">
                <label for="<?= 'link_border_radius_' . $row->biolink_block_id ?>"><i class="fa fa-fw fa-border-all fa-sm text-muted mr-1"></i> <?= l('create_biolink_link_modal.input.border_radius') ?></label>
                <select id="<?= 'link_border_radius_' . $row->biolink_block_id ?>" name="border_radius" class="form-control">
            ...
            <div class="form-group">
                <label for="<?= 'link_border_style_' . $row->biolink_block_id ?>"><i class="fa fa-fw fa-border-none fa-sm text-muted mr-1"></i> <?= l('create_biolink_link_modal.input.border_style') ?></label>
                <select id="<?= 'link_border_style_' . $row->biolink_block_id ?>" name="border_style" class="form-control">
